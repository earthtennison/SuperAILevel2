...
def getAngle(a, b, c):  # a-> b -> c; find angle at b; cosine law
    bLength = getLength(a, c)
    aLength = getLength(b, c)
    cLength = getLength(a, b)
    temp = math.acos((aLength ** 2 + cLength ** 2 - bLength ** 2) / (2 * aLength * cLength))
    return round(math.degrees(temp))
...
class TfPoseEstimator:
    ...
    @staticmethod
    def draw_arm(npimg, humans, imgcopy=False):
        leftAngle = 0
        rightAngle = 0
        if imgcopy:
            npimg = np.copy(npimg)
        ...
        for human in humans:
            ...
            if 2 in human.body_parts.keys() and 3 in human.body_parts.keys() and 4 in human.body_parts.keys():
                rightAngle = getAngle(centers[2], centers[3],centers[4])
            else:
                rightAngle = 0

            if 5 in human.body_parts.keys() and 6 in human.body_parts.keys() and 7 in human.body_parts.keys():
                leftAngle = getAngle(centers[5], centers[6], centers[7])
            else:
                leftAngle = 0

        return npimg,rightAngle,leftAngle
